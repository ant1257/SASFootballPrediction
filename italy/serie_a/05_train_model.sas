	...
	data &data._&target;
		set &data._&target;
			...
			%if &target = home_win %then %do;
				if p_1 < 0.3972699123 then segment = 'RED';
				else if p_1 < 0.6169322739 then segment = 'YELLOW';
				else segment = 'GREEN';
			%end;
			
			%else %do;
				if p_1 < 0.194455177 then segment = 'RED';
				else if p_1 < 0.4004153276 then segment = 'YELLOW';
				else segment = 'GREEN';
			%end;
