...
%macro get_real_score_data;
  %let web_site = https://www.fussballdaten.de/italien/&season/&round/;
  ...
  data temp_&season._&round;
    set whole_page_&season._&round;
      ...
      i = 1;
      var1 = cat('HREF="/ITALIEN/', "&season", '/', "&round", '/STATISTIK/"');
      ...
      if index(upcase(line), upcase(compress(var1)))
      	...
      	do while(index(upcase(temp_var), '" CLASS="" HREF="/ITALIEN/'));
        	start_symbol = index(upcase(temp_var), '" CLASS="" HREF="/ITALIEN/');
        	end_symbol = index(upcase(temp_var), 'SERIE A)"><SPAN>');

        	temp_value = substr(temp_var, start_symbol, end_symbol - start_symbol);
        	temp_var = substr(temp_var, end_symbol + 16);
        	i + 1;
...
/*   %do i = 1 %to 18; */
  ...
  proc datasets lib = work nolist;
  	delete temp_: results_: tm_: whole_: team_: fixtures_: ;
  run;
