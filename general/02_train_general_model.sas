...
/* proc fastclus data = train_01 maxclusters = 4 out = train_02 outstat = general.clust_stat; */
/* 	var &std_names; */
/* run; */
...
%macro train(target);
	proc logistic data = train_03 outmodel = general.model_&target;
		model &target(event = '1') = &std_names home_cluster away_cluster /
		maxiter = 50
		selection = forward
		slentry=0.15;
	run;
