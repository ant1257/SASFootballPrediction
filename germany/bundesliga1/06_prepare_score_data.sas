  ...
  data temp_&season._&round;
    set whole_page_&season._&round;
      ...
      if index(upcase(line), upcase(compress(var1)))
      	...
      	do while(index(upcase(temp_var), '" CLASS="" HREF="/BUNDESLIGA/'));
        	start_symbol = index(upcase(temp_var), '" CLASS="" HREF="/BUNDESLIGA/');
        	end_symbol = index(upcase(temp_var), 'BUNDESLIGA)"><SPAN>');

        	temp_value = substr(temp_var, start_symbol, end_symbol - start_symbol);
        	temp_var = substr(temp_var, end_symbol + 19);
        	i + 1;
...
/*   outfile = '/folders/myfolders/footbal/germany/bundesliga1/csv/score.csv' */
/*   dbms = csv */
/*   replace; */
/* run; */
