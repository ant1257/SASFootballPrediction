...
/* CHANGE MANUALY UNTILL ROUND 20 */
/* %let round = 18; */

%macro get_real_score_data;
  %let web_site = https://www.fussballdaten.de/spanien/&season/&round/;
  ...
  data temp_&season._&round;
    set whole_page_&season._&round;
      ...
      i = 1;
      var1 = cat('HREF="/SPANIEN/', "&season", '/', "&round", '/STATISTIK/"');
      ...
      if index(upcase(line), upcase(compress(var1)))
      	...
      	do while(index(upcase(temp_var), '" CLASS="" HREF="/SPANIEN/'));
        	start_symbol = index(upcase(temp_var), '" CLASS="" HREF="/SPANIEN/');
        	end_symbol = index(upcase(temp_var), 'PRIMERA DIVISIÓN)"><SPAN>');

        	temp_value = substr(temp_var, start_symbol, end_symbol - start_symbol);
        	temp_var = substr(temp_var, end_symbol + 25);
        	i + 1;
...
/*   outfile = '/folders/myfolders/footbal/spain/laliga/csv/score.csv' */
/*   dbms = csv */
/*   replace; */
/* run; */
