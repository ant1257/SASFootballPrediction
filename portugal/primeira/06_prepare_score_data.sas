  ...
  data temp_&season._&round;
    set whole_page_&season._&round;
      ...
      if index(upcase(line), upcase(compress(var1)))
      	...
      	do while(index(upcase(temp_var), '" CLASS="" HREF="/PORTUGAL/'));
        	start_symbol = index(upcase(temp_var), '" CLASS="" HREF="/PORTUGAL/');
        	end_symbol = index(upcase(temp_var), 'LIGA NOS)"><SPAN>');

        	temp_value = substr(temp_var, start_symbol, end_symbol - start_symbol);
        	temp_var = substr(temp_var, end_symbol + 17);
        	i + 1;
...
%macro add_missing;
	...
	proc datasets lib = work nolist;
  		delete temp_: points_: results_: tm_: whole_: team_: fixtures_: ;
  	run;
%mend add_missing;
